...
class Flow:
    ...
    def get_data(self) -> dict:
        ...
        #     "timestamp": packet_time.get_time_stamp(),
            ...
            "fwd_pkt_len_min": float(packet_length.get_min(PacketDirection.FORWARD)),##In layer-2
            ...
            "bwd_pkt_len_min": float(packet_length.get_min(PacketDirection.REVERSE)),##In layer-2
            ...
            "pkt_len_min": packet_length.get_min(),##In layer-2
            ...
            "fwd_act_data_pkts": packet_count.has_payload(PacketDirection.FORWARD),##In layer-2
            ...
            "bwd_iat_min": float(backward_iat["min"]),##In layer-2
            "bwd_iat_mean": float(backward_iat["mean"]),##In layer-2
            "bwd_iat_std": float(backward_iat["std"]),##In layer-2
        #     # Flags statistics
        ...
        #     "bwd_urg_flags": flag_count.has_flag("URG", PacketDirection.REVERSE),
            "fin_flag_cnt": flag_count.has_flag("FIN"),##In layer-2
        #     "syn_flag_cnt": flag_count.has_flag("SYN"),
        #     "rst_flag_cnt": flag_count.has_flag("RST"),
            "psh_flag_cnt": flag_count.has_flag("PSH"),## In layer-1
            "ack_flag_cnt": flag_count.has_flag("ACK"),## In layer-1
        ...
        #     # Response Time
            ...
            "active_max": float(active_stat["max"]),##In layer-2
            "active_min": float(active_stat["min"]),##In layer-2
            "active_mean": float(active_stat["mean"]),##In layer-2
            "active_std": float(active_stat["std"]),##In layer-2
            "idle_max": float(idle_stat["max"]),## In layer-2
            "idle_min": float(idle_stat["min"]),## In layer-2
            "idle_mean": float(idle_stat["mean"]),## In layer-2
            "idle_std": float(idle_stat["std"]),## In layer-2
    ...
    def update_subflow(self, packet):
        ...
        if (packet.time - (last_timestamp / float("1e6"))) > constants.CLUMP_TIMEOUT:
